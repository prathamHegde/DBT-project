...
parsed_df = df.select(from_json(col("value"), schema).alias("data")).select("data.*") \
    .select("iss_position.latitude", "iss_position.longitude", "timestamp","message","traceback")
...
def write_to_mysql(row):
    ...
    query = "INSERT INTO location_table(latitude, longitude, timestamp, message, traceback) \
             VALUES (%s, %s, %s, %s, %s)"

    # Execute the SQL query with the row data
    cursor.execute(query, (row.latitude, row.longitude, row.timestamp, row.message, row.traceback))
