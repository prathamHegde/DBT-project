...
schema = StructType([
    ...
    StructField("iss_acceleration", DoubleType()),
    StructField("iss_semi_major_axis", DoubleType()),
])
...
parsed_df = df.select(from_json(col("value"), schema).alias("data")).select("data.*") \
    .select("iss_position.latitude", "iss_position.longitude", "timestamp","message","traceback","iss_acceleration","iss_semi_major_axis")
...
def write_to_mysql(row):
    ...
    query = "INSERT INTO acc_table(latitude, longitude, timestamp, message, traceback,iss_acceleration,iss_semi_major_axis) \
             VALUES (%s, %s, %s, %s, %s, %s,%s)"

    # Execute the SQL query with the row data
    cursor.execute(query, (row.latitude, row.longitude, row.timestamp, row.message, row.traceback, row.iss_acceleration, row.iss_semi_major_axis))
